...
class MyCustomTrainer:
    ...
    def _parse_optimizers_schedulers(
        ...
        """Recursively parses the output of configure_optimizers."""
        ...
        if isinstance(configure_optim_output, L.fabric.utilities.types.LRScheduler):
            scheduler_config = _lr_sched_defaults.copy()
            scheduler_config["scheduler"] = configure_optim_output
            return None, scheduler_config

        # single lr scheduler config
        if isinstance(configure_optim_output, Mapping):
            scheduler_config = _lr_sched_defaults.copy()
            scheduler_config.update(configure_optim_output)
            return None, scheduler_config

        # list or tuple
        if isinstance(configure_optim_output, (list, tuple)):
            if len(configure_optim_output) == 2:
                optimizers, schedulers = configure_optim_output
                # 如果优化器是列表，取第一个
                ...
                if isinstance(schedulers, (list, tuple)):
                    scheduler_config = _lr_sched_defaults.copy()
                    scheduler_config.update(schedulers[0])
                else:
                    scheduler_config = _lr_sched_defaults.copy()
                    scheduler_config.update(schedulers)
