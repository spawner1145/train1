...
logger = logging.get_logger('eugebooru_ipa')
...
def get_ata_caption(
    ...
    **kwargs,
    ...
    caption_type = random.choice(captions)
    if caption_type == 'caption':
        return get_ata_tags(img_md, dataset_hook, **kwargs)
    elif caption_type == 'description':
        return get_description(img_md, dataset_hook, **kwargs)
...
def get_ata_tags(
    ...
    **kwargs,
    ...
    safety: List[str] = img_md.get('rating_tags')[0]
    quality: List[str] = img_md.get('danbooru_quality_tags')[0]
    aesthetic_score_1: float = img_md.get('aesthetic_score_1',None)
    # original_size = img_md.get('original_size')
    # date = img_md.get('date')
    year: str = img_md.get('year')
    period: str = img_md.get('year_tags')[0]
