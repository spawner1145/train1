...
import sys
sys.path.append("/root/autodl-tmp/naifu")
...
def main():
    ...
    model_path = {
        # "stage_a": "/root/autodl-tmp/StableCascade/models/stage_a.safetensors",
        "stage_b": "/root/autodl-tmp/StableCascade/models/stage_b_bf16.safetensors",
    ...
    model_path = types.SimpleNamespace(**model_path)
    config_file = "/root/autodl-tmp/naifu/config/train_cascade_stage_c.yaml"
    with open(config_file, "r") as f:
        config = yaml.load(f, Loader=yaml.SafeLoader)
    
    model = StableCascadeModel(model_path, config, 'cuda')
    image = model.sample(prompt, size=size, generator=generator)
