...
def setup(fabric: pl.Fabric, config: OmegaConf) -> tuple:
    ...
    dataset_class = get_class(config.dataset.get("name", "data_loader.arrow_load_stream.TextImageArrowStream"))
...
class SupervisedFineTune(StableDiffusionModel):    
    def forward(self, batch):
        ...
        if True:
            ...
            latents = self.encode_first_stage(batch["pixels"].contiguous().to(self.first_stage_model.dtype))
        ...
        if advanced.get("offset_noise") and random.random() < 0.35:
